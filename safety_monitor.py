...
class SystemSafetyMonitor:
    ...
    def _check_failed_logins(self) -> Dict:
        ...
        try:
            # Try different approaches to get failed login data
            failed_attempts = 0
            
            # Method 1: Try journalctl for SSH
            try:
            ...
            # Method 2: Try auth.log if journalctl failed
                    ...
                    for log_path in auth_log_paths:
                        if os.path.exists(log_path):
                            with open(log_path, 'r') as f:
                                # Get last hour of logs (rough estimation)
    ...
    def check_system_services(self) -> Dict:
        ...
        for service in critical_services:
            try:
                ...
                results[service] = {
                    'status': status,
                    'alert': status not in ['active', 'inactive']  # inactive is OK for optional services
                }
                
                # Special handling for optional services
                if service in ['fail2ban', 'ufw'] and status == 'inactive':
                    results[service]['alert'] = False  # These services might be intentionally disabled
                    
            except subprocess.TimeoutExpired:
                results[service] = {'error': 'Service check timeout', 'alert': True}
            except FileNotFoundError:
                results[service] = {'error': 'systemctl not found', 'alert': True}
            except Exception as e:
                results[service] = {'error': f'Service check failed: {str(e)}', 'alert': True}
...
def main():
    ...
    if args.setup:
        ...
        print("Default configuration file will be created on first run.")
        print("Please edit config.ini to customize settings.")
        return
        ...
        try:
            ...
            while True:
                try:
                    ...
                    time.sleep(check_interval)  # Continue monitoring even after errors
